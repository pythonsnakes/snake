...
class GameModel:
    def __init__(self, size):
        ...
        self.second_snake = SnakeModel(Point(1, 38), size)
        self.food = None
        self.food = []
        self.generate_food()
    ...
    def check_bump_into_self(self):
        head1 = self.first_snake.body[0]
        head2 = self.second_snake.body[0]
        if self.first_snake.current_dir != None:
            collide_seg1 = head1 + self.first_snake.current_dir
            if collide_seg1 in self.first_snake.body:
                self.game_result = GameResult.SNAKE2_WIN
        if self.second_snake.current_dir != None:
            collide_seg2 = head2 + self.second_snake.current_dir
            if collide_seg2 in self.second_snake.body:
                self.game_result = GameResult.SNAKE1_WIN
        if self.first_snake.current_dir != None and \
                self.second_snake.current_dir != None:
            collide_seg1 = head1 + self.first_snake.current_dir
            collide_seg2 = head2 + self.second_snake.current_dir
            if collide_seg1 in self.first_snake.body \
                    and collide_seg2 in self.second_snake.body:
                self.game_result = GameResult.DRAW
